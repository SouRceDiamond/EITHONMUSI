...
@Client.on_message(filters.command(["ينج"], prefixes=f"{HNDLR}"))
async def ping(client, m: Message):
    await m.delete()
    start = time()
    current_time = datetime.utcnow()
    m_reply = await m.reply_text("جاري حساب سرعه البينج ⚡️ ")
    delta_ping = time() - start
    uptime_sec = (current_time - START_TIME).total_seconds()
    uptime = await _human_time_duration(int(uptime_sec))
    await m_reply.edit(
        f"<b>-›  بينج</b> `{delta_ping * 1000:.3f} ms` \n<b>-›  الوقت</b> - `{uptime}`"
    )
...
🛠 هذه هي قائمـة اوامر سـورس الماس
- أوامر المستخدمين: 
...
• !بينج - لعرض سرعه النت للبوت

• !الاوامر - لعرض اوامر سورس ميوزك الماس
